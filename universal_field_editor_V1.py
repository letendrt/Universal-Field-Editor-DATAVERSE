...
file_directory = '/INSERT/YOUR/DIRECTORY/HERE/File.csv'

api_token_origin = "YOUR_API_KEY_HERE"                                             # Enter API key as string
url_base_origin = 'https://borealisdata.ca'                                        # Input base origin url as string (e.g., https://demo.borealisdata.ca)
        ...
        if assess[0] not in existing_field_typeNames and assess[0] != 'doi':         # If the existing record does not hold the current field typeName (and is not a DOI)
            ...
            if fields['typeName'] in change_area and fields['typeName'] in master_list[1]:           # If the field typeName is in the existing record and is in the compound category
                ...
                if field_format == False:                                            # If the function output is False
                    ...
                    field['value'] = field_format                                    # Insert new entry in previous value field
                    API_push(field, doi)                                             # Push to API
...
def file_loader():
    ...
    df = pd.read_csv(file_directory)                                           # Read csv file and create dataframe
    ...
    print(master_list)                                                         # Print the master list of lists

    with (open(file_directory, newline='', encoding='utf-8-sig') as csvfile):  # Read csv file
        ...
        for row in reader:                                                     # For each row
            ...
            pub_stat = update_metadata_name(latest_version, row, doi, header, directory, master_list)       # Assign fuinction output to the variable 'pub_stat' - optional, only if for auto-publish
